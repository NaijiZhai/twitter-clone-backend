...
class NewsFeedApiTests(TestCase):
    ...
    def setUp(self):
        self.linghu = self.create_user('linghu', email='<EMAIL>')
        self.linghu_client = APIClient()
        self.linghu_client.force_authenticate(self.linghu)

        self.dongxie = self.create_user('dongxie', email='<EMAIL>')
        self.dongxie_client = APIClient()
        self.dongxie_client.force_authenticate(self.dongxie)

        # create followings and followers for dongxie
        for i in range(2):
            follower = self.create_user('dongxie_follower{}'.format(i), email='<EMAIL1>{}'.format(i))
            Friendship.objects.create(from_user=follower, to_user=self.dongxie)
        for i in range(3):
            following = self.create_user('dongxie_following{}'.format(i),email='<EMAIL>{}'.format(i))
    ...
    def test_list(self):
        ...
        # 关注之后可以看到别人发的
        self.linghu_client.post(FOLLOW_URL, data = {'from_user_id': self.linghu.id, 'to_user_id': self.dongxie.id},
                                format='json')
